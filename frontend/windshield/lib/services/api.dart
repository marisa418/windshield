...
class Api extends ChangeNotifier {
  ...
  Future<BSheetLog> getBalanceSheetLog() async {
    try {
      ...
      final data = (res.data as List).map((i) => BSheetLog.fromJson(i)).toList();
      return data.last;
    } catch (e) {
      return BSheetLog(id: 0, timestamp: DateTime.now(), assetValue: 0, debtValue: 0, bsheetId: '');
    }
  }
  ...
  Future<bool> addAsset(String source, double recentVal, String catId) async {
    try {
      ...
      await dio.post(
        '/api/asset/',
        data: {"source": source == '' ? null : source, "recent_value": recentVal, "cat_id": catId},
      );
      return true;
    } catch (e) {
      return false;
